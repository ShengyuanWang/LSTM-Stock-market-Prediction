...
```{r}
df %>% 
  mutate(index = ifelse(type %in% c("test", "train"), index - 1, index)) %>%
  filter(index >= 1500,
         index <= 1550) %>% 
  ggplot(aes(x = index,
             y = value,
...
```{r}
# Save the predicted numbers as X-1
index <- c(1,2,3,4)
...
index <- c(1821,1822,1823,1824,1825,1826)
...
write.csv(bitcoin_result, "prediction/bitcoin_LSTM_result_modified.csv")
```
